...
final class ProducerHomeViewModel {
    ...
    // MARK: - Initialization
    init(swiftDataService: SwiftDataService, networkService: NetworkService? = nil) {
    ...
    func loadData() {
        Task {
            ...
            do {
                // Load user profile
                if let profile = try swiftDataService.getCurrentUserProfile() {
                    userProfile = profile
                    
                    // Load recent diagnoses (last 5)
                    recentDiagnoses = try swiftDataService.fetchDiagnosisHistory(for: profile, limit: 5)
                }
            } catch {
    ...
    func getGreeting() -> String {
        ...
        let displayName = userProfile?.userName ?? "Productor"
    ...
    func getTotalDiagnoses() -> Int {
        ...
        do {
            return try swiftDataService.fetchDiagnosisHistory(for: profile, limit: 1000).count
        } catch {
            return 0
        }
    }

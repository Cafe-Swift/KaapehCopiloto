...
final class SwiftDataService {
    ...
    /// Obtiene el historial de diagnósticos de un usuario específico
    func fetchDiagnosisHistory(for profile: UserProfile, limit: Int = 50) throws -> [DiagnosisRecord] {
        ...
        var descriptor = FetchDescriptor<DiagnosisRecord>(
            predicate: #Predicate { record in
                record.userProfile?.userId == userId
            },
    ...
    func calculateCPM() throws -> Double {
        ...
        let cpm = (totalConfidence / Double(allRecords.count)) * 100.0
        // Redondear a 2 decimales para evitar errores de precisión de punto flotante
        return (cpm * 100).rounded() / 100
    }
}
